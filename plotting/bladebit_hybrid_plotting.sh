#!/bin/bash
###############################################################################
# Script Name:       bladebit_hybrid_plotting.sh
...
#
# Usage:             ./bladebit_hybrid_plotting.sh
#
# Requirements:
#   - At least 480 GiB of temporary space
#   - Sudo access
...
START_TIME=$(date +%s)
sudo-g5k /tmp/bladebit-build/bladebit -f "$FARMER_KEY" -p "$POOL_KEY" -n 17 -z 5 diskplot -t1 "$TMP_DIR" "$PLOT_DIR" &
PLOT_PID=$!
